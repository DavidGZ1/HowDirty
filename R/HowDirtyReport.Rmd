...
# read_conta_results(file_report_skyline = params$PeakAreasContaminantsFile) %>% 
#   names


read_samples_annotation <- function(file_samples_annotation){
  read.csv(file_design) %>% 
...
# file_design = params$AnnotationFile
...
raw_conta <- read_conta_results(file_report_skyline = params$PeakAreasContaminantsFile)
...
reps_samples_annot <- unique(samples_annot$ReplicateName)
...
if(length(reps_only_samples_annot) > 0) {
  message(paste("Warning, the following samples from the Annotation file were not found in the Skyline results and won't be processed: ", paste(reps_only_samples_annot, collapse = ", ")))
} 
```

```{r echo=FALSE}
#Merge samples_annot and contamination, assign thresholds

conta <-
  inner_join(samples_annot, raw_conta, by = "ReplicateName") %>% 
  mutate(RawFile = ReplicateName,
         ReplicateName = factor(ReplicateName, levels = unique(samples_annot$ReplicateName)),
         Sample = factor(Sample, levels = unique(samples_annot$Sample)),
         Condition = factor(Condition, levels = unique(samples_annot$Condition))) %>% 
...
# conta_summ_sample <-
#   full_join(samples_annot %>%
#               mutate(RawFile = ReplicateName,
#                      ReplicateName = Sample), conta_summ_sample, by = "ReplicateName") #Pending: rename Sample and replicatename in samples_annot
