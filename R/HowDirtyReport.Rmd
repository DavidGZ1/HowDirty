...
if(params$RefThresholdsFile == FALSE){
  ...
  ref_conta_tshd <- get_simple_thresholds_analyte(raw_conta)
...
#Check if all samples are included in both results and annotation file
check_samples_in_results(raw_conta, samples_annot[c(1:10), ])
...
```{r merging_dfs, echo=FALSE}

##Merge samples_annot and contamination, multiply by dilution_factor, assign thresholds
conta_annotated <- annotate_conta_samples(df_conta = raw_conta,
                                          df_samples_annot = samples_annot,
                                          multiply_dilution_factor = params$MultiplyDilutionFactor)
#Adding and evaluating thresholds
# conta <- annotate_conta_thresholds(conta_annotated, ref_conta_tshd)
# 
# conta <- conta_annotated %>% 
