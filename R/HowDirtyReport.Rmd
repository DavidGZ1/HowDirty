...
plot_abundance_analytegroup_list <- lapply(levels(conta$AnalyteGroup[c(1:3)]), function(x){
  plot_abundance(filter(conta, AnalyteGroup == x)) %>% 
        ggplotly(width = get_w(), height = get_h()) %>% 
    htmltools::tagList()
}
)
names(plot_abundance_analytegroup_list) <- levels(conta$AnalyteGroup[c(1:3)])
