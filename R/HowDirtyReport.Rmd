...
* The data set contained the following percentages of samples associated to each risk level
...
# fluidRow(
#   column(2,radioButtons(inputId =  "abundance_scale_sample_total",
#                         label = "Scale",
...
#                         width = "90%")
#   ),
#   column(2, radioButtons(inputId =  "order_sample_total",
#                          label = "Order_x",
...
#                          width = "90%")
#   ),
#   column(3, checkboxInput(inputId =  "facet_condition_total",
#                         label = "Facet by Condition",
#                         value  = TRUE,
#                         width = "90%")
#   # column(4, radioButtons(inputId =  "facet_condition_total",
#   #                        label = "Facet by Condition",
...
#     # layout_ggplotly_label_margin(x = -0.05, y = -0.02) #fixes label margin problem, but we have to adapt the margins to avoid it going out of margins
#   
# })

dot_plot_sample_risk_total <-  plot_sample_risk_total(conta_summ_sample, order_x = "Abundance") 
...
```{r}
conta_summ_analytegroup_sample %>% 
  ggplot(aes(y = AnalyteGroup, x = Abundance_total)) +
...
```{r plot_AnalyteGroupArea, results='asis', echo=FALSE, fig.height=8, fig.width=10, warning=FALSE}
# renderPlotly({
...
# solution to print ggplotly inside asis based on https://stackoverflow.com/questions/61906480/how-to-display-ggplotly-plots-with-dynamically-created-tabs-and-for-loops
...
plot_abundance_analytegroup_list <- lapply(levels(conta$AnalyteGroup), function(x){
  plot_abundance(filter(conta, AnalyteGroup == x)) %>% 
        ggplotly(width = get_w(), height = get_h()) %>% 
    htmltools::tagList()
}
)
names(plot_abundance_analytegroup_list) <- levels(conta$AnalyteGroup)

for (i in 1:length(plot_abundance_analytegroup_list)) {
  cat("### ", names(plot_abundance_analytegroup_list)[i],"\n")
    print(htmltools::tagList(plot_abundance_analytegroup_list[[i]]))
  # plot_abundance_analytegroup_list[i]  %>%  print()
  # ggplotly(plot_abundance_analytegroup_list[[i]])
  cat('\n\n')
}

# plot_abundance_analytegroup_list[[1]]
