---
title: "Getting started with HowDirty"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Getting started with HowDirty}
...
HowDirty quantifies molecular contamination in LC-MS experiments. It reads peak
area exports from [Skyline](https://skyline.ms) and produces a self-contained HTML
report with risk-scored tables and plots for each contaminant, sample, and
...
**1. Skyline peak-area export** — a CSV produced by Skyline's
*File → Export → Report* dialog using a contaminant report definition. The
file must contain columns for protein (contaminant name), peptide, replicate
...
`Sample`, `Condition`, and `DilutionFactor` (all optional; defaults are applied
if omitted). Add any extra metadata columns you need (e.g. Batch, Instrument).
...
Returns a data frame with one row per contaminant × replicate. The key derived
column is `Abundance = TotalAreaMS1 / TotalIonCurrentArea` (TIC-normalised).
...
Optional: set `multiply_dilution_factor = TRUE` if your annotation includes
meaningful dilution factors that should scale the abundances.

### 4. Set contamination thresholds

**Without a reference dataset** — use uniform thresholds derived from built-in
...
Reference thresholds give more meaningful risk scores by comparing your samples
...
Each row is assigned `RiskLevel` (0–6) and a human-readable `Risk` label.
See `RISK_LABELS` for the full mapping.

### 6. Summarise

`summarize_conta()` accepts any grouping variables:
...
Each summary includes `Abundance_{min, quantile25, median, quantile75, quantile90, max, total}`.
...
All plot functions return `ggplot2` objects and accept a `rotate_x` argument
...
This renders the bundled RMarkdown template and writes a self-contained HTML
file plus an Excel summary workbook. Use `get_report_template()` to copy the
template to your working directory for customisation.
...
If you don't have R installed, the official Docker image lets you run HowDirty
with no local setup. All input files must be in a single directory that is
...
Fill in the `Sample`, `Condition`, and `DilutionFactor` columns before the
next step.
...
Every flag has an equivalent environment variable (`DATASET`, `FILE_PEAK_AREAS`,
`FILE_ANNOTATION`, etc.) — useful for scripting or CI pipelines:
...
- Full function reference: `help(package = "HowDirty")`
- Parameterised RMarkdown template: `get_report_template()`
- Multi-dataset comparison: `?compare_howdirty`
