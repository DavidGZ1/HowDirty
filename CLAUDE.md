...
NAMESPACE and all `man/*.Rd` files are **generated by roxygen2** — never edit them by hand. When adding new exported functions:
1. Add `@param`, `@return`, and `@export` tags to the roxygen2 comment block in the `R/` source file.
2. Wrap **every** `@examples` block in `\dontrun{}` — no exceptions. R CMD CHECK runs unwrapped examples and this package has no real Skyline data available in the check environment.
3. Run `devtools::document()` to regenerate NAMESPACE and the matching `man/fn_name.Rd`.
4. The `\usage` and `\arguments` sections are derived directly from the function signature and `@param` tags — keep those in sync in the R source, not in the Rd files.

### pkgdown site

The site is configured in `_pkgdown.yml` and deployed automatically to GitHub Pages on every push to `main` via `.github/workflows/pkgdown.yaml`. To build locally: `pkgdown::build_site()`. Site URL: https://davidgz1.github.io/HowDirty/
