...
conta_summ_sample <-
  ...
  # select(Condition, Sample, ReplicateName, Abundance_total = Abundance_total, Risk, RiskLevel) %>%
...
#   summarise(Abundance_total = round(sum(Abundance, na.rm=TRUE), 4), .groups = "drop") %>%
#   mutate(Risk =
...
#   conta_summ_sample %>%
#   select(-Abundance_total) %>%
#     # rename(TotalAbundance_Total = Abundance_total) %>%
...
#                values_to = "Abundance_median") %>% # this was not the median, it was the sum
...
print_plot_or_plotly(dot_plot_conta_summ_condition_risk_analytegroup, height_plotly = 400, width_plotly = 600)
```
...
plot_bp_abundance_analytegroup_sample <-
  plot_abundance_analytegroup(conta_summ_analytegroup_sample,
                              x = Abundance_median,
                              variable = Condition)
...
```{r plot_AnalyteGroupPseudoChromato, results='asis', echo=FALSE, fig.height=5, fig.width=8, warning=FALSE}
plot_seudochromatogram_analytegroup_list <-
  lapply(levels(conta$AnalyteGroup)[c(1:params$nTopContaminantGroups)],
         function(x){
           plot_pseudochromatogram(filter(conta, AnalyteGroup == x)) %>% 
             ggplotly(width = get_w(), height = get_h()*0.7) %>%
